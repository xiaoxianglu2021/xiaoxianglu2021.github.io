...
layout:     post
title:      快速搭建个人博�?
subtitle:   手把手教你在半小时内搭建自己的个人博�?如果不踩坑的话🙈🙊�?
date:       2017-02-06
author:     BY
header-img: img/init/post-bg-re-vs-ng2.jpg
catalog: true
tags:
    - Blog
---
...
> 正所谓前人栽树，后人乘凉�?
> 
> 感谢[Huxpro](https://github.com/huxpro)提供的博客模�?
> 
> [我的的博客](http://qiubaiying.top)

# 前言
�?Jekyll �?GitHub Pages 中间踩了许多坑，终于把我的个人博客[BY Blog](http://qiubaiying.top)搭建出来了。。�?
...
要使�?GitHub Pages，首先你要注册一个[GitHub](https://github.com/)账号，GitHub 是全球最大的同性交友网�?吐槽下程序员~)，你值得拥有�?
...
注册完成后搜�?`qiubaiying.github.io` 进入[我的仓库](https://github.com/qiubaiying/qiubaiying.github.io)
...
<p id = "Rename"></p>
修改仓库名为 `你的Github账号�?github.io`，然�?Rename
...
这时你在在浏览器中输�?`你的Github账号�?github.io` 例如:`baiyingqiu.github.io`
...
header-img: img/init/post-bg-rwd.jpg  	#显示在首页的背景图片
email: You@gmail.com	
description: "You Blog"  			 #网站介绍
keyword: "BY, BY Blog, 柏荧的博�? qiubaiying, 邱柏�? iOS, Apple, iPhone" #关键�?
...
sidebar-avatar:/img/init/avatar-by.JPG      # 你的个人头像 这里你可以改成我在img文件夹中的两张备用照片 img/init/avatar-m 或 avatar-g
```
#### 社交账号
展示你的其他社交平台
...
新加入了**简�?*，`jianshu_id` 在你打开你的简书主页后的地址如：`http://www.jianshu.com/u/e71990ada2fd`中，后面这一串数字：`e71990ada2fd `
...
这个 **Username**  就是我们 `_config.yml` �?`disqus_username`
...
> 很对人反�?Disqus 评论插件加载不出来，因为 Disqus 在国内加载缓慢，所以我新集成了 Gitalk 评论插件（感谢[@FeDemo](https://github.com/FeDemo)的推荐），喜欢折腾的朋友可以看这篇：[《为博客添加 Gitalk 评论插件》](http://qiubaiying.top/2017/12/19/%E4%B8%BA%E5%8D%9A%E5%AE%A2%E6%B7%BB%E5%8A%A0-Gitalk-%E8%AF%84%E8%AE%BA%E6%8F%92%E4%BB%B6/)�?我已经在`_config.yml` 配置就好了，只需要填写参数可以了�?

#### 网站统计

集成�?[Baidu Analytics](http://tongji.baidu.com/web/welcome/login) �?[Google Analytics](http://www.google.cn/analytics/)，到各个网站注册拿到track_id替换下面的就可以�?
...
利用 Github网站 ，我们可以不用学习[git](https://git-scm.com/)，就可以轻松管理自己的博�?

对于轻车熟路的程序猿来说，使用git管理会更加方便。。�?

## 创建
文章统一放在网站根目录下�?`_posts` 的文件夹中�?
...
文件的格式是 `.md` �?[**MarkDown**](http://sspai.com/25137/) 文件�?

我们的博客文章格式采用是 **MarkDown**+ **YAML** 的方式�?

[**YAML**](http://www.ruanyifeng.com/blog/2016/07/yaml.html?f=tt) 就是我们配置 `_config`文件用的语言�?

[**MarkDown**](http://sspai.com/25137/) 是一种轻量级的「标记语言」，很简单。[花半个小时看一下](http://sspai.com/25137)就能熟练使用�?
...
subtitle:   Hello World, Hello Blog #副标�?
date:       2017-02-06 				# 时间
author:     BY 						# 作�?
header-img: img/init/post-bg-2015.jpg 	#这篇文章标题背景图片
catalog: true 						# 是否归档
tags:								#标签
    - 生活
---
...
在首页可以看到这些特色标签，当你的文章出现相同标签（默认相同�?*标签数量大于1**），才会自动生成�?
...
建站的初期，博客比较少，若你想直接在首页生成比较多的标签。你可以�?`_congfig.yml`中找到这段：

```
# Featured Tags
featured-tags: true                     # 是否使用首页标签
featured-condition-size: 1              # 相同标签数量大于这个数，才会出现在首�?
```

将其修改为`featured-condition-size: 0`, 这样只有一个标签时也会出现在首页了�?

相反，当你博客比较多，标签也很多时，这时你就需要改�?`1` 甚至�?`2` 了�?


# 自定义域�?

搭建好博客之�?你可能不想直接使�?[baiyingqiu.github.io](http://baiyingqiu.github.io) 这么长的博客域名�? 想换成想 [qiubaiying.top](http://qiubaiying.top) 这样简短的域名。那我们开始吧�?

#### 购买域名
首先，你必须购买一个自己的域名�?

我是在[阿里云](https://wanwang.aliyun.com/domain/?spm=5176.8006371.1007.dnetcndomain.q1ys4x)购买的域�?
...
�?*阿里�?* app也可以注册域名，域名的价格根据后缀的不同和域名的长度而分，比如我这个 `qiubaiying.top` 的域名第一年才只要4元~

域名尽量选择短一点比较好记住，注意，不能选择中文域名，比�?`张三.top` ,GitHub Pages **无法处理中文域名**，会导致你的域名在你的主页上使用�?
...
一个主机记录为 `www`,代表可以解析 `www.qiubaiying.top`的域�?
...
细心地你会发现所有人的博客都解析�?`151.101.100.133` 这个IP�?
...
最后一步，只需要修�?我们github仓库下的 **CNAME** 文件�?
...
若你对博客模板进行修改，你就要看�?Jekyll 的[开发文档](http://jekyll.com.cn),是中文文档哦，对英语一般的朋友简直是福利啊（比如说我😀）�?

还要学习 **Git** �?**GitHub** 的工作机制了及使用�?

你可以先看看这个[git教程](http://www.liaoxuefeng.com/wiki/0013739516305929606dd18361248578c67b8067c8c017b000/)，对git有个初步的了解后，那么相信你就能将自己图片传到GitHub仓库上，或者可以说掌握�?**使用git管理自己的GitHub仓库** 的技能呢�?

对于轻车熟路的程序猿来说，这篇教程就算就结束了，因为下面的内容对于你们来�?so eazy~

但相信很多小白都一脸懵逼，那我们继续👇�?

# 利用GithHub Desktop管理GitHub仓库

[GithHub Desktop](https://desktop.github.com/) �?**GithHub** 推出的一款管理GitHub仓库的桌面软件，换句话说就是将你�?*Github**上的文件同步到本地电脑上，并将修改后的文件同步到**Github**远程仓库�?

#### 下载

点击图片进入下载页面，选择对应的平台进行下�?
...
当你对仓库文件夹的文件下进行修改、添加或删除时，都可以在 **GitHub Desktop** 中看�?

例如我在 `img` 中添加了一张图�?`avatar-demo.png` 添加了一张图�?

就可以在看到**GitHub Desktop**显示了我的修�?

保存修改只要�?**Commit to master**，然后可以写上你的修改说�?
...
想上传头像，背景，或者是删掉你不要的图片（我的头像😏）已经�?so eazy了吧~

#### 注意
你在 **GitHub** 网站上进�?**Commit** 操作后，需要在**GitHub Desktop**上按一�?**同步按键** 才能同步网站上的修改到你的本地�?
...
看不�?HTML 标签？没关系，对照着修改就好了~ 还有注意这个有中英介�?
...
最近有人往我的远程仓库不停�?**push**，一天连收几十封邮件！例如像这样�?
...
> 注：下面的操作是�?**Mac** 终端进行的�?
> **Windows** 环境下的配置请参�?[@梦幻之云](http://www.jianshu.com/u/a13e7484dc21) 提供�?[这篇文章](https://agcaiyun.cn/2017/09/10/%E6%90%AD%E5%BB%BA%E4%B8%AA%E4%BA%BA%E5%8D%9A%E5%AE%A2/)�?
...
# => 打开浏览�?http://localhost:4000
```


这段命令创建了一个默认的 `jekll` 网站，然后在本机�?4000 窗口展示。聪明的你应该发现怎么做了吧~

安装 `jekyll`�?`jekyll bundler`
...
你就可以�?<http://127.0.0.1:4000/> 看到你的博客，你对本地博客的修改都会在这个地址进行显示，这大大提高了对博客的配置效率�?

使用`ctrl+c`就可以停�?**serve**

# Star

若本教程顺利帮你搭建了自己的个人博客，请不要 **害羞**，给我的 [github仓库](https://github.com/qiubaiying/qiubaiying.github.io) 点个 **star** 吧！

因为最近发�?Fork 将近破百，加上直�?Clone 仓库的，保守估计已经帮助上百人成功的搭建了自己的博客，~~可是 Star 却仅仅只�?**12**！可能还是做的不够好吧！~~现在已经破百了，感谢大家的Star�?

### **别无他求，点�?[Star](https://github.com/qiubaiying/qiubaiying.github.io) �?*�?
...
在博�?`img` 目录下找到并替换 `favicon.ico` 这个图标即可，图标尺寸为`32x32`�?
...
所以，要在 MacDown 中插入图片，这张图片就需要上传到图床（网上），然后在�?
用这张图片的URL�?
...
使用方法很简单，直接拖动图片�?P 图标上，或者选中图片按快捷键 `�?U`，就能请示上传�?
